...
const map = L.map('map', { zoomControl: false, center: [48.3, -100.8], zoom: 8 });
...
function showIntel(type, name) {
    ...
    const h = document.getElementById('intel-name');
    const c = document.getElementById('intel-content');
...
function closeIntel() { document.getElementById('intel-window').style.display = 'none'; }
...
function refreshPreview(id) {
    ...
    // --- CHANGE: Only clear the preview for THIS specific unit ID ---
    if (unitPreviews[id]) {
    ...
    // Add to the global preview layer AND track it locally
...
map.on('click', function (e) {
    ...
    impactLayer.clearLayers(); // Clear old target markers
    L.circleMarker(e.latlng, { color: 'var(--yellow)', radius: 3 }).addTo(impactLayer);
...
function launch(id) {
    ...
    // Ensure preview is visible upon launch
    refreshPreview(id);
...
function detonate(lat, lng, y, id) {
    ...
    // --- CHANGE: Clean up this unit's specific preview group ---
    if (unitPreviews[id]) {
        previewLayer.removeLayer(unitPreviews[id]);
        delete unitPreviews[id];
    }

    // Permanent impact rings (these stay forever)
    L.circle([lat, lng], { color: 'orange', fillOpacity: 0.6, radius: Math.pow(y, 0.4) * 110, stroke: false }).addTo(impactLayer);
    L.circle([lat, lng], { color: 'red', fillOpacity: 0.15, radius: Math.pow(y, 0.33) * 750 }).addTo(impactLayer);
